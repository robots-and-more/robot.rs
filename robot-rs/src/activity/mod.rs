...
impl<T> MaybeReferred<T> {
    ...
    pub async fn try_take(
        &mut self,
        priority: Priority,
        take_back: oneshot::Sender<oneshot::Sender<T>>,
    ) -> Option<T> {
...
pub struct System<T> {
    ...
    default_activity_factory: Arc<
        std::sync::RwLock<
            Option<
                Arc<dyn Fn(&mut T) -> Pin<Box<dyn Future<Output = ()> + '_ + Send>> + Send + Sync>,
            >,
        >,
    >,
}
...
impl<T> System<T> {
    pub fn new(system: T) -> Self {
        Self {
            storage: Mutex::new(MaybeReferred::Owned(system)),
            default_activity_factory: Arc::new(std::sync::RwLock::new(None)),
        }
    }

    pub fn set_idle_activity<
        F: Fn(&mut T) -> Pin<Box<dyn Future<Output = ()> + '_ + Send>> + Send + Sync + 'static,
    >(
        &self,
        f: F,
    ) {
    ...
    pub async fn on_activity_finished(self: Arc<Self>, mut sys: T) {
        let factory = { self.default_activity_factory.read().unwrap().clone() };
        match factory {
            Some(factory) => {
                let (tx, rx) = oneshot::channel();
    ...
    where
        F: FnOnce(&mut T) -> Pin<Box<dyn Future<Output = O> + '_ + Send>>,
    {
        let (tx_take, rx_take) = oneshot::channel();

        let val = { self.storage.lock().await.try_take(priority, tx_take).await };
...
macro_rules! perform {
    ($system:expr, $priority:expr, $func:path) => {
        perform!($system, $priority, |x| futures::future::FutureExt::boxed(
            async move { $func(x).await }
        ))
    };

            ...
            maybe_flip(
                ...
                ProfileState {
                    displacement: cur.displacement
                        + (cur.velocity + 0.5 * self.max_acceleration * t) * t,
            ...
            maybe_flip(
                ...
                ProfileState {
                    displacement: goal.displacement
                        - (goal.velocity + 0.5 * self.max_acceleration * remaining_time)
                            * remaining_time,
    ...
    fn calculate(&mut self, input: Displacement<Vel, Time>, time: Time) -> Self::Output {
        match self.last_state.clone() {
            Some(last) => {
                ...
                self.controller.calculate(input, time)
                    + self.feedforward.calculate(next_state, time)
            }
            None => {
